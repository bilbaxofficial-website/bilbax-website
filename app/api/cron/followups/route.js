...
// ANALYTICS LOGGING -----------------------------------------------------
// message_logs is what the Analytics dashboard reads (every row = 1
// "triggered" event; dm_sent=true = 1 "DM sent"; collected_value set =
// 1 "lead"). Previously this table was only ever written from the
// "data collected" branch far below, so any automation WITHOUT a
// collect-email/phone step (direct send, or follow-gate-only) never
// wrote a row at all - that's why analytics stayed at zero no matter
// how many comments/DMs actually went through.
//
// logTrigger() is called exactly once, the moment an automation first
// matches (comment or story reply) - this is the "triggered" event.
// markFlowCompleted() is called later to update that SAME row (not
// insert a new one) once the final link actually goes out, or once a
// collect-field reply comes in - so a single user flow is always 1 row,
...
async function logTrigger({ automation, triggerUserId, triggerUsername, matchedKeyword, dmSent }) {
  ...
  if (error) console.error("❌ MESSAGE_LOGS INSERT ERROR:", error.message);
...
async function markFlowCompleted({ automationId, senderId, dmSent, collectedValue }) {
  ...
  if (findErr) console.error("❌ MESSAGE_LOGS LOOKUP ERROR:", findErr.message);
  if (!row) {
    console.log("⚠️ ANALYTICS: no existing trigger row found to update for", senderId, "- inserting fallback row");
  ...
  const update = {};
  if (dmSent !== undefined) update.dm_sent = !!dmSent;
  if (collectedValue !== undefined) update.collected_value = collectedValue;

  const { error: updErr } = await supabase.from("message_logs").update(update).eq("id", row.id);
  if (updErr) console.error("❌ MESSAGE_LOGS UPDATE ERROR:", updErr.message);
  else console.log("📊 ANALYTICS: flow completion logged for row", row.id, update);
}
    ...
    for (const entry of entries) {
      ...
      for (const change of changes) {
        ...
        const matched = eligibleForThisPost.find((a) => {
          ...
          const found = kwList.find((kw) => commentText.toLowerCase().includes(String(kw).toLowerCase().trim()));
      ...
      for (const msg of messaging) {
        ...
        if (isStoryReply && !pending && rawText) {
          ...
          if (storyAutomations && storyAutomations.length > 0) {
            ...
            const matched = storyAutomations.find((a) => {
              ...
              const found = kwList.find((kw) => rawText.toLowerCase().includes(String(kw).toLowerCase().trim()));
              if (found) matchedKeyword = found;
              return !!found;
            });
        ...
        if (pending.status === "awaiting_second_click" && (textPayload === "SECOND_FOLLOW_CONFIRMED" || textPayload.toLowerCase().includes("yes"))) {
           ...
           if (afterFollow === "awaiting_data") {
             ...
             await markFlowCompleted({ automationId: automation.id, senderId, dmSent: sentFinal });
           }
           continue;
        }
        ...
        if (pending.status === "awaiting_data" && rawText) {
          ...
          // Update the SAME message_logs row created when this flow was
          // first triggered, instead of inserting a second row - so one
          // user flow = one "triggered" count in analytics, not two.
